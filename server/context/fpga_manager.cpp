...
static int synthesize_uio_devnodes() {
    ...
    for (const auto& u : fs::directory_iterator(cls)) {
        ...
        if (!f.good()) {
            koheron::print_fmt<ERROR>("FpgaManager: cannot read {}/dev\n", u.path());
            continue;
        }
        ...
        if (access(node.c_str(), F_OK) != 0) {
            if (mknod(node.c_str(), S_IFCHR | 0660, makedev(maj, min)) != 0) {
                koheron::print_fmt<ERROR>("FpgaManager: mknod({}) failed ({}: {})\n", node, errno, strerror(errno));
            } else {
                ++made;
            }
        }
...
static void bind_and_log_uio_mappings() {
    ...
    for (const auto& devdir : devs) {
        ...
        if (!uio_node.empty()) {
            koheron::print_fmt<INFO>("FpgaManager: '{}' mapped to /dev/{}\n", devname, uio_node);
        } else {
            koheron::print_fmt<ERROR>("FpgaManager: '{}' has no /dev/uio mapping yet\n", devname);
        }
    }
}
...
FpgaManager::FpgaManager() {
    ...
    koheron::print<PANIC>("FpgaManager: cannot identify bitstream loading mechanism\n");
    ::_exit(EXIT_FAILURE);
}
...
int FpgaManager::check_bitstream_loaded(const fs::path& fprog_done_path, char expected) {
    ...
    if (!f) {
        koheron::print_fmt<ERROR>("FpgaManager: check_bitstream_loaded: open('{}') failed ({}: {})\n",
                                  fprog_done_path, errno, strerror(errno));
        koheron::print<PANIC>("FpgaManager: open FPGA status failed\n");
        return -1;
    }
    std::array<char,1> b{};
    int r = read(fileno(f), b.data(), 1);
    if (r != 1) {
        koheron::print_fmt<ERROR>("FpgaManager: check_bitstream_loaded: read('{}') returned {}\n", fprog_done_path, r);
        fclose(f);
    ...
    koheron::print_fmt<ERROR>("FpgaManager: check_bitstream_loaded: got '{}', expected '{}'\n", b[0], expected);
    koheron::print<PANIC>("FpgaManager: PL configuration failed\n");
    return -1;
}
...
int FpgaManager::copy_firmware() {
    ...
    if (!fs::exists(live_instrument_dirname)) {
        koheron::print_fmt<ERROR>("FpgaManager: copy_firmware: source dir '{}' missing\n", live_instrument_dirname);
        return -1;
    }
    if (!fs::exists(libfw)) {
        std::error_code ec;
        ...
        if (ec) {
            koheron::print_fmt<ERROR>("FpgaManager: copy_firmware: mkdir '{}' failed: {}\n", libfw, ec.message());
            return -1;
        }
    }
    ...
    if (ec1) {
        koheron::print_fmt<ERROR>("FpgaManager: copy_firmware: pl.dtbo copy failed: {}\n", ec1.message());
        return -1;
    }
    ...
    if (ec2) {
        koheron::print_fmt<ERROR>("FpgaManager: copy_firmware: '{}' copy failed: {}\n", bitbin, ec2.message());
        return -1;
    }
    return 0;
}
...
int FpgaManager::clean_up_previous_overlays() {
    ...
    if (fs::directory_iterator(kOverlaysRoot) != fs::directory_iterator{}) {
        koheron::print<PANIC>("FpgaManager: overlays still present; cannot proceed\n");
        return -1;
    }
    return 0;
}
...
int FpgaManager::mount_configfs() {
    if (!fs::exists(kCfgfsRoot)) {
        ...
        if (ec) {
            koheron::print_fmt<ERROR>("FpgaManager: mount_configfs: mkdir '{}' failed: {}\n", kCfgfsRoot, ec.message());
            return -1;
        }
    }
    if (!fs::exists(kCfgfsDT)) {
        if (mount("none", kCfgfsRoot.c_str(), "configfs", 0, nullptr) < 0) {
            koheron::print_fmt<ERROR>("FpgaManager: mount_configfs: mount failed ({}: {})\n", errno, strerror(errno));
            return -1;
        }
    }
    return 0;
...
int FpgaManager::setup_overlay_path() {
    if (mount_configfs() < 0) {
        koheron::print<ERROR>("FpgaManager: setup_overlay_path: mount_configfs failed\n");
        return -1;
    }
    if (clean_up_previous_overlays() < 0) {
        koheron::print<ERROR>("FpgaManager: setup_overlay_path: cleanup failed\n");
        return -1;
    }
    std::error_code ec;
    fs::create_directories(overlay_path, ec);
    if (ec) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_overlay_path: mkdir '{}' failed: {}\n", overlay_path, ec.message());
        return -1;
    }
    if (!fs::exists(overlay_path)) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_overlay_path: '{}' not present after mkdir\n", overlay_path);
        return -1;
    }
    return 0;
}
...
int FpgaManager::setup_fmanager_flags() {
    ...
    if (!xflag) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_fmanager_flags: open('{}') failed ({}: {})\n",
                                  fmanager_flags, errno, strerror(errno));
        return -1;
    }
    if (fwrite("0", 1, 1, xflag) != 1) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_fmanager_flags: write('{}') failed ({}: {})\n",
                                  fmanager_flags, errno, strerror(errno));
        fclose(xflag);
        return -1;
    }
...
int FpgaManager::write_overlay() {
    ...
    if (!f) {
        koheron::print_fmt<ERROR>("FpgaManager: write_overlay: open('{}') failed ({}: {})\n",
                                  overlay, errno, strerror(errno));
        return -1;
    }
    const std::string echo = "pl.dtbo\n";
    if (fwrite(echo.c_str(), echo.size(), 1, f) != 1) {
        koheron::print_fmt<ERROR>("FpgaManager: write_overlay: write('{}') failed\n", overlay);
        fclose(f);
        return -1;
    }
    fclose(f);
...
int FpgaManager::load_bitstream_overlay() {
    ...
    const auto bitbin = fs::path{INSTRUMENT_NAME ".bit.bin"};
    koheron::print_fmt<INFO>("FpgaManager: Loading {}\n", bitbin);

    if (copy_firmware() < 0) {
        koheron::print_fmt<ERROR>("FpgaManager: copy_firmware() failed — could not install 'pl.dtbo' or '{}' into /lib/firmware\n", bitbin);
        return -1;
    }
    if (setup_overlay_path() < 0) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_overlay_path() failed — cannot prepare overlay dir '{}'\n", overlay_path);
        return -1;
    }
    if (setup_fmanager_flags() < 0) {
        koheron::print_fmt<ERROR>("FpgaManager: setup_fmanager_flags() failed — cannot write flags at '{}'\n", fmanager_flags);
        return -1;
    }
    if (write_overlay() < 0) {
        koheron::print_fmt<ERROR>("FpgaManager: write_overlay() failed — cannot write 'pl.dtbo' to '{}/path'\n", overlay_path);
        return -1;
    }

    int rc = check_bitstream_loaded(overlay_fpga_done, 'a');
    if (rc != 0) {
        koheron::print_fmt<ERROR>("FpgaManager: overlay apply failed — expected status 'a' in '{}'\n", overlay_fpga_done);
        return rc;
    }
...
int FpgaManager::read_bitstream_data(std::vector<char>& bitstream_data) {
    ...
    if (!f) {
        koheron::print_fmt<ERROR>("FpgaManager: read_bitstream_data: open('{}') failed ({}: {})\n",
                                  filename, errno, strerror(errno));
        return -1;
    }

    if (fseek(f, 0, SEEK_END) != 0) {
        koheron::print_fmt<ERROR>("FpgaManager: read_bitstream_data: fseek end failed\n");
        fclose(f);
        return -1;
    }
    long sz = ftell(f);
    if (sz <= 0) {
        koheron::print_fmt<ERROR>("FpgaManager: read_bitstream_data: invalid size {}\n", sz);
        fclose(f);
        return -1;
    }
    rewind(f);
    ...
    if (fread(bitstream_data.data(), bitstream_data.size(), 1, f) != 1) {
        koheron::print_fmt<ERROR>("FpgaManager: read_bitstream_data: fread failed\n");
        fclose(f);
        return -1;
    }
    fclose(f);
...
int FpgaManager::load_bitstream_xdevcfg() {
    ...
    if (!xdevcfg) {
        koheron::print_fmt<ERROR>("FpgaManager: load_bitstream_xdevcfg: open('{}') failed ({}: {})\n",
    ...
    if (read_bitstream_data(buf) < 0) {
        koheron::print<ERROR>("FpgaManager: load_bitstream_xdevcfg: read_bitstream_data failed\n");
        fclose(xdevcfg);
        return -1;
    }

    if (fwrite(buf.data(), buf.size(), 1, xdevcfg) != 1) {
        koheron::print_fmt<ERROR>("FpgaManager: load_bitstream_xdevcfg: write '{}' failed\n", xdev);
        fclose(xdevcfg);
        return -1;
    }
    fclose(xdevcfg);
    ...
    if (rc != 0) {
        koheron::print_fmt<ERROR>("FpgaManager: xdevcfg apply failed — expected status '1' in '{}'\n", xdev_fpga_done);
    }
    return rc;
}
