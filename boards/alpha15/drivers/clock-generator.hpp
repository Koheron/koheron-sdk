...
class ClockGenerator
{
  ...
  private:
    ...
    // double fs_adc0; // ADC0 sampling frequency (Hz)
    // double fs_adc1; // ADC1 sampling frequency (Hz)
    ...
    int configure(uint32_t cfg_mode, uint32_t clkin_select, const std::array<uint32_t, clock_cfg::num_params>& clk_cfg_) {
        ...
        // R0: CLKOUT
        uint32_t CLKout0_DIV = clk_cfg[clock_cfg::CLKout0_DIV];
        ...
        // R3: FPGA clock
        uint32_t CLKout3_DIV = clk_cfg[clock_cfg::CLKout3_DIV];
        uint32_t CLKout3_DDLY = 0;  // Digital delay
        uint32_t CLKout3_PD = 0;    // Power down

        // R4: ADC1 clock
        uint32_t CLKout4_DIV = clk_cfg[clock_cfg::CLKout4_DIV];
        uint32_t CLKout4_DDLY = 0;  // Digital delay (0 => + 2ns, 8 => +750 ps, 9 => +350 ps, 10 => -50 ps, 15 => -2.1 ns)
        uint32_t CLKout4_PD = 0;    // Power down

        // R5: EXP_CLK clock
        uint32_t CLKout5_DIV = clk_cfg[clock_cfg::CLKout5_DIV];
        ...
        // R28
        uint32_t PLL2_R = clk_cfg[clock_cfg::PLL2_R];
        ...
        uint32_t PLL2_N_CAL = clk_cfg[clock_cfg::PLL2_N];

        // R30
        uint32_t PLL2_P = clk_cfg[clock_cfg::PLL2_P];
        uint32_t PLL2_N = clk_cfg[clock_cfg::PLL2_N];
        ...
        fs_adc[1] = f_vco / CLKout4_DIV;
        fs_dac = f_vco / CLKout2_DIV;

        if (fs_adc[0] > fs_adc_max || fs_adc[1] > fs_adc_max || fs_dac > fs_dac_max) {
        ...
        ctx.log<INFO>("Clock generator - Ref: %s, VCO: %f MHz, ADC0: %f MHz, ADC1: %f MHz, DAC: %f MHz\n",
                      clock_cfg::clkin_names[clkin_select].data(),
        ...
        if (cfg_mode == SAMPLING_FREQ_SET || cfg_mode == CFG_ALL) {
            ...
            phase_shift(clk_cfg[clock_cfg::MMCM_PHASE_SHIFT]);
        }
