...
const std::array<float, 8> Ltc2157::get_calibration(uint32_t adc, uint32_t channel) {
    if (adc >= 2) {
        log<ERROR>("Ltc2157::get_calibration: Invalid adc\n");
...
int32_t Ltc2157::set_calibration(uint32_t adc, uint32_t channel, const std::array<float, 8>& new_coeffs) {
    if (adc >= 2) {
        log<ERROR>("Ltc2157::set_calibration: Invalid adc\n");
        return -1;
    }
...
float Ltc2157::get_input_voltage_range(uint32_t adc, uint32_t channel) const {
...
float Ltc2157::get_gain(uint32_t adc, uint32_t channel) const {
    if (adc >= 2) {
        log<ERROR>("Ltc2157::get_gain: Invalid adc\n");
        return std::numeric_limits<float>::quiet_NaN();
    }
...
float Ltc2157::get_offset(uint32_t adc, uint32_t channel) const {
    if (adc >= 2) {
        log<ERROR>("Ltc2157::get_offset: Invalid adc\n");
        return std::numeric_limits<float>::quiet_NaN();
    }
