...
namespace clock_cfg {
    ...
    constexpr std::array<std::array<uint32_t, num_params>, num_configs> configs = {fs_200MHz, fs_250MHz, fs_240MHz, fs_100MHz};
}
...
class ClockGenerator
{
  ...
  private:
    ...
    int configure(uint32_t cfg_mode, uint32_t clkin_select, const std::array<uint32_t, clock_cfg::num_params>& clk_cfg_) {
        ...
        uint32_t CLKout2_DDLY = 0;  // Digital delay
        ...
        if (fs_adc[0] > fs_max || fs_adc[1] > fs_max) {
        ...
        ctx.log<INFO>("Clock generator - Ref: %s, VCO: %f MHz, ADC0: %f MHz, ADC1: %f MHz\n",
