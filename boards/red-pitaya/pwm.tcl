...
proc add_pwm {module_name ps_name clk offset pwm_width} {
  ...
  connect_bd_net [get_bd_pins clk] [get_bd_pins /$clk]
	...
	for {set i 0} {$i < $n_pwm} {incr i} {
      ...
      [list NBITS $pwm_width] \
		  [list clk clk rst /rst_${ps_name}_125M/peripheral_reset pwm_out concat_pwm/In$i]
		cell xilinx.com:ip:xlslice:1.0 pwm_slice_$i \
		  [list DIN_WIDTH 1024 DIN_FROM [expr $pwm_width-1+32*$i+$offset] DIN_TO [expr 32*$i+$offset]] \
