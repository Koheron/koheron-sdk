...
, dac(dvm_, dac_brams)
{
    ...
    raw_data[0] = dvm.get_ptr<int32_t>(adc_map[0]);
    raw_data[1] = dvm.get_ptr<int32_t>(adc_map[1]);

    set_averaging(false); // Reset averaging

    // set tvalid delay to 19 * 8 ns
    config_map.write<ADDR_OFF>(19 << 2);
    set_n_avg_min(0);
    set_clken_mask(true);
                                                     ...
                                                     uint32_t index_low, uint32_t index_high)
{
    ...
    uint32_t avg_on = bool(status_map.read<AVG_ON_OUT0_OFF>());
    if (avg_on) {
...
{
    if (avg_on) {
        config_map.set_bit<AVG0_OFF, 0>();
        config_map.set_bit<AVG1_OFF, 0>();
    } else {
        config_map.clear_bit<AVG0_OFF, 0>();
        config_map.clear_bit<AVG1_OFF, 0>();
    }
}
...
void Oscillo::_wait_for_acquisition()
{
    ...
    do {
        if (n_avg_min_ > 0) {
            ...
            uint32_t n_avg_min = config_map.read<N_AVG_MIN0_OFF>();
    ...
    } while (status_map.read<AVG_READY0_OFF>() == 0
             || status_map.read<AVG_READY1_OFF>() == 0);
}
