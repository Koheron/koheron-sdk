                                                     ...
                                                     uint32_t index_low, uint32_t index_high)
{
    ...
    uint32_t avg_on = bool(dvm.read<AVG_ON_OUT0_OFF>(status_map));
    if (avg_on) {
...
void Oscillo::_wait_for_acquisition()
{
    ...
    do {
        if (n_avg_min_ > 0) {
            ...
            uint32_t n_avg_min = dvm.read<N_AVG_MIN0_OFF>(config_map);
    ...
    } while (dvm.read<AVG_READY0_OFF>(status_map) == 0
             || dvm.read<AVG_READY1_OFF>(status_map) == 0);
}
