...
class Eeprom
{
  public:
    ...
    uint32_t read(uint32_t addr) {
        dvm.write(config_map, SPI_IN_OFF, (READ_OPCODE << 7) + (addr << 1));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
        std::this_thread::sleep_for(100us);
        return dvm.read(status_map, SPI_OUT_OFF);
    }

    void write_enable() {
        dvm.write(config_map, SPI_IN_OFF, (EWEN << 5));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }

    void erase(uint32_t addr) {
        dvm.write(config_map, SPI_IN_OFF, (ERASE_OPCODE << 7) + (addr << 1));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }

    void write(uint32_t addr, uint32_t data_in) {
        dvm.write(config_map, SPI_IN_OFF, (data_in << 16) + (WRITE_OPCODE << 7) + (addr << 1));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }

    void erase_all() {
        dvm.write(config_map, SPI_IN_OFF, (ERAL << 5));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }

    void write_all(uint32_t data_in) {
        dvm.write(config_map, SPI_IN_OFF, (data_in << 16) + (WRAL << 5));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }

    void erase_write_disable() {
        dvm.write(config_map, SPI_IN_OFF, (EWDS << 5));
        dvm.set_bit(config_map, SPI_IN_OFF, 0);
    }
