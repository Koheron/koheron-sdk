...
class FIFOReader
{
  public:
    ...
    void set_map(MemMapID fifo_map_){
        ...
        dvm.write(fifo_map.load(), RDFR_OFF, 0xA5); // Reset FIFO
    }
...
void FIFOReader<N>::acquisition_thread_call(uint32_t acq_period)
{
    ...
    while (is_acquiring.load()) {
        ...
        fifo_length.store((dvm.read(fifo_map.load(), RLR_OFF) & 0x3FFFFF) >> 2);

        if (fifo_length.load() > 0) {
            std::lock_guard<std::mutex> guard(ring_buff_mtx);
            for (uint32_t i=0; i<fifo_length.load(); i++) {
                ring_buffer[index.load()] = dvm.read(fifo_map.load(), RDFD_OFF);
