...
set intercon_idx 0
set nfifos [get_parameter n_fifos]
...
source $project_path/tcl/decimator.tcl

for {set i 0} {$i < $nfifos} {incr i} {
  decimator::create decimator$i $fir_coeffs

  set idx [add_master_interface $intercon_idx]

  connect_bd_intf_net [get_bd_intf_pins decimator$i/S_AXI] \
                      [get_bd_intf_pins [set interconnect_${intercon_idx}_name]/M${idx}_AXI]

  connect_cell decimator$i {
    tdata          adc_selector/tdata
    tvalid         adc_selector/tvalid
    ps_clk         [set ps_clk$intercon_idx]
    clk            adc_dac/adc_clk
    s_axis_aresetn rst_adc_clk/peripheral_aresetn
    m_axis_aresetn [set rst${intercon_idx}_name]/peripheral_aresetn
    rate           [ps_ctl_pin cic_rate$i]
  }
}

assign_bd_address
set segs [get_bd_addr_segs /${::ps_name}/Data/SEG_adc_axis_fifo_Mem0*]

for {set i 0} {$i < $nfifos} {incr i} {
  set memory_segment [lindex $segs $i]
...
# FIFO interrupts
set base 2
set nports [expr {$base + $nfifos}]
set_property CONFIG.NUM_PORTS $nports [get_bd_cells concat_interrupts]

for {set i 0} {$i < $nfifos} {incr i} {
  set pin_from [get_bd_pins [format "decimator%d/interrupt" $i]]
  set pin_to   [get_bd_pins [format "concat_interrupts/In%d" [expr {$base + $i}]]]
  connect_bd_net $pin_from $pin_to
}
