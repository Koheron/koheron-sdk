/// ADC DAC BRAM FEEDBACK driver
///
/// (c) Koheron

#ifndef __DRIVERS_ADC_DAC_BRAM_FEEDBACK_HPP__
#define __DRIVERS_ADC_DAC_BRAM_FEEDBACK_HPP__
...
class AdcDacBramFeedback
{
  public:
    AdcDacBramFeedback(Context& ctx_)
    : ctx(ctx_)
    ...
    , clockgen(ctx.get<ClockGenerator>())
    {
        ...
        ctx.log<INFO>("AdcDacBramFeedback: t_acq = {%li, %li} us\n", t_acq[0], t_acq[1]);
    }
    ...
    void integrate(bool state) {
        state ? ctl.set_bit<reg::integrate, 0>()
              : ctl.clear_bit<reg::integrate, 0>();
    }
...
#endif // __DRIVERS_ADC_DAC_BRAM_FEEDBACK_HPP__