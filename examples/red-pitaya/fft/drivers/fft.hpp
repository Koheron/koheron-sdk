...
class FFT
{
  public:
    ...
    const std::array<int32_t, prm::n_adc> get_adc_raw_data(uint32_t n_avg) {
        if (n_avg <= 1) {
            return { ((static_cast<int32_t>(sts.read<reg::adc0>()) + 8192) % 16384) - 8192,
                     ((static_cast<int32_t>(sts.read<reg::adc1>()) + 8192) % 16384) - 8192 };
        } else {
            int32_t adc0 = 0;
            int32_t adc1 = 0;

            for (size_t i=0; i<n_avg; i++) {
                adc0 += ((static_cast<int32_t>(sts.read<reg::adc0>()) + 8192) % 16384) - 8192;
                adc1 += ((static_cast<int32_t>(sts.read<reg::adc1>()) + 8192) % 16384) - 8192;
            }
    ...
    void set_dds_freq(uint32_t channel, double freq_hz) {
        ...
        constexpr double factor = (uint64_t(1) << 32) / fs_adc;
 ...
 private:
    ...
    // XXX Since Red Pitaya is 1 MOhm input, maybe V^2/Hz are better ...
...
inline void  FFT::psd_acquisition_thread() {
    ...
    while (psd_acquisition_started) {
        ...
        while (cycle_index >= previous_cycle_index) {
            auto sleep_time = std::chrono::nanoseconds((prm::n_cycles - cycle_index) * 2048 * 8);
            if (sleep_time > 1ms) {
