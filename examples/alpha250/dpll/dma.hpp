...
class Dma
{
  public:
    Dma() {
        auto& clk_gen = services::require<rt::DriverManager>().get<ClockGenerator>();
    ...
    auto& get_data() {
        auto& dma = services::require<rt::DriverManager>().get<DmaS2MM>();
        dma.start_transfer(mem::ram_addr, sizeof(int32_t) * prm::n_pts);
        dma.wait_for_transfer(dma_transfer_duration);
        auto& ram = services::require<hw::MemoryManager>().get<mem::ram>();
